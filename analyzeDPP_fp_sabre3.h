...
void analyzeDPP_fp_sabre3(int runNumber, int segNumber) 
{
  ...
  if(segNumber==0)  
	   std::cout << "ch vs gain found as follows.." << std::endl;
   while(!in.eof())
   {
     in>>ch_1>>temp1>>temp2;
     ...
     if(segNumber==0)
     {
        std::cout << ch_1 << " " << sabre_gainmatch[ch_1] << " ";
        if(ch_1%16==15)
		   std::cout << "\n";
     }
   }
...
//  return;
   ...
   static TH1D *tcheckFront = new TH1D("tcheckFront",
	"Focal Plane Detector Checking (front plane)",240,-coincidenceWindow/1.e3 - 400,coincidenceWindow/1.e3 + 400);
   static TH1D *tcheckBack = new TH1D("tcheckBack",
	"Focal Plane Detector Checking (back plane)",240,-coincidenceWindow/1.e3 - 400,coincidenceWindow/1.e3 + 400);
   static TH1D *anodeScintTime = new TH1D("anodeScintTime",
        "Anode / Scintillator Coincidence Time",240,-coincidenceWindow/1.e3 - 400,coincidenceWindow/1.e3 + 400);
...
//   ca = new TCanvas("dt-checks","dt-checks",0,0,1200,800);
   ca.Divide(3,1,0,0);
   ca.cd(1);
...
//   dtsabreanode->Rebin(8);
   ...
   //static TCanvas *ccheck = new TCanvas("ccheck","ccheck");
   ca.cd(2);
//   tcheckFront->Rebin(8);
   tcheckFront->Draw();
   ...
   //static TCanvas *cscintanode = new TCanvas("Scint-Anode-Coinc","Scint-Anode-Coinc");
   ca.cd(3);
...
/*   compassTree->Delete();
   ...
   tcheckFront->Delete();
   tcheckBack->Delete();
   anodeScintTime->Delete();
   syncCheckHistogram->Delete();
   builtTree->Delete();
*/
   builtFile->Close();
   compassFile->Close();
