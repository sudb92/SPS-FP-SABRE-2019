...
int test_map()
{
  ...
  ifstream in; 
  std::map<int,sabrechan> map1;   //map[0-127] should return the sabrechan instance with all the properties
  ...
  for(int i=0; i<128; i++)
  {
    std::cout << map1[i].detID << " " << map1[i].side_pos.first << " " << map1[i].side_pos.second <<std::endl;
  }
  return 1;
}
